...
const API_BASE_URL = "https://website-checker-web.onrender.com";
...
const DashboardPage = () => {
    ...
    const handleAddWebsite = async (e) => {
        ...
        if (newWebsite && !websites.some(site => site.url === newWebsite)) {
            const newWebsites = [...websites, { id: Date.now(), url: newWebsite }];
            setWebsites(newWebsites.sort((a, b) => a.url.localeCompare(b.url)));
            await fetch(`${API_BASE_URL}/save`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ urls: newWebsites.map(w => w.url) }),
            });
            setNewWebsite('');
            fetchData();
        }
    ...
    const handleRemoveWebsite = async (idToRemove) => {
        ...
        await fetch(`${API_BASE_URL}/save`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ urls: updatedWebsites.map(w => w.url) }),
        });
        fetchData();
    };
    ...
    const handleRunAllChecks = async () => {
        ...
        await fetch(`${API_BASE_URL}/check-all`);
        fetchData();
    ...
    const handleFileUpload = async (e) => {
        ...
        if (file) {
            ...
            await fetch(`${API_BASE_URL}/import`, { method: "POST", body: formData });
            fetchData();
        }
    };
